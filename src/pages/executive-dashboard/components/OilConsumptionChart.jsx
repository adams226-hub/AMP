...
export default function OilConsumptionChart({ data = [], weekStart, weekEnd, periodLabel }) {
  ...
  return (
    <div className="rounded-xl border p-4 md:p-6" style={{ background: "var(--color-card)", borderColor: "var(--color-border)", boxShadow: "var(--shadow-sm)" }}>
      <div className="flex items-start justify-between mb-5">
        ...
        {reste > 0 && (
          <span className="text-xs px-2 py-1 rounded-full flex-shrink-0" style={{ background: "rgba(217,119,6,0.10)", color: COLOR }}>
      ...
      {isEmpty ? (
        <div className="flex flex-col items-center justify-center h-48 gap-2">
          <span style={{ color: "var(--color-muted-foreground)", fontSize: 13 }}>
            Aucune sortie huile cette semaine
          </span>
        </div>
      ) : (
            ...
            >
              ...
              <Bar dataKey="consommation" radius={[0, 4, 4, 0]} barSize={22}>
                {top.map((_, i) => {
                  ...
                  return <Cell key={i} fill={`rgba(217,119,6,${intensity.toFixed(2)})`} />;
                })}
                <LabelList
