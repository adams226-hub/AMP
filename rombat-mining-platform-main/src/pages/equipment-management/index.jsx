...
export default function EquipmentManagement() {
  ...
  const handleAddEquipment = async () => {
    ...
    try {
      ...
      setNewEquipment({ name: '', type: '', model: '', serial_number: '', location: 'Koro', status: 'active' });
  ...
  const handleAddOperation = async () => {
    try {
      ...
      if (!newOperation.equipment_id) {
        toastError('Veuillez sélectionner un équipement dans la liste');
        return;
      }

      const selectedEquipment = equipment.find(e => e.id === newOperation.equipment_id);
      if (!selectedEquipment) {
        toastError('Équipement sélectionné invalide');
        return;
      }

      const operationToAdd = {
        date: newOperation.date,
        equipment_id: newOperation.equipment_id,
        shift: newOperation.shift,
        machine_type: selectedEquipment.type || '',
        status: 'functional',
  ...
  const handleDeleteOperationLog = async () => {
    if (!confirmDeleteLogId) return;
    try {
      const { error } = await miningService.deleteOperationLog(confirmDeleteLogId);
      if (error) throw error;
  ...
  return (
    <AppLayout userRole={user?.role} userName={user?.full_name} userSite="Amp Mines et Carrieres">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b" style={{ borderColor: "var(--color-border)" }}>
                    ...
                    <th className="text-left p-3 text-xs font-medium" style={{ color: "var(--color-muted-foreground)" }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {operationLogs.length === 0 ? (
                    ...
                    operationLogs.map((log) => (
                      <tr key={log.id} className="border-b" style={{ borderColor: "var(--color-border)" }}>
                        ...
                        <td className="p-3 text-sm" style={{ color: "var(--color-foreground)" }}>{log.counter_start != null ? Number(log.counter_start).toLocaleString() : '-'}</td>
                        <td className="p-3 text-sm" style={{ color: "var(--color-foreground)" }}>{log.counter_end != null ? Number(log.counter_end).toLocaleString() : '-'}</td>
                        <td className="p-3 text-sm" style={{ color: "var(--color-foreground)" }}>{log.distance != null ? Number(log.distance).toFixed(1) : '0'} km</td>
                        <td className="p-3">
                          <button
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: "var(--color-muted-foreground)" }}>
                  Cette action est irréversible. Si des données sont liées, l'équipement sera passé en statut "".
                </p>
              </div>
            </div>
      ...
      {/* Modal Confirmation Suppression Log */}
      {confirmDeleteLogId && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="rounded-xl p-6 w-full max-w-sm" style={{ background: "var(--color-card)" }}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-base font-semibold" style={{ color: "var(--color-foreground)" }}>Supprimer cette opération ?</h3>
                <p className="text-xs mt-0.5" style={{ color: "var(--color-muted-foreground)" }}>
            ...
            <div className="flex gap-3 justify-end">
              <Button variant="outline" onClick={() => setConfirmDeleteLogId(null)}>Annuler</Button>
              <Button variant="destructive" onClick={handleDeleteOperationLog}>Supprimer</Button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="rounded-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto" style={{ background: "var(--color-card)" }}>
            ...
            <div className="space-y-4">
              ...
              {/* Sélection Équipement + Type machine */}
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1" style={{ color: "var(--color-foreground)" }}>
                    Équipement *
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setNewOperation({
                        ...
                        code_agent: selectedEquipment ? selectedEquipment.name : '',
                        machine_type: selectedEquipment ? selectedEquipment.type : ''
                      });
                    }}
                    className="w-full p-2 rounded border"
                    style={{ borderColor: "var(--color-border)", background: "var(--color-background)", color: "var(--color-foreground)" }}
                  >
                    ...
                    {equipment.map(eq => (
                      <option key={eq.id} value={eq.id}>
                        {eq.name} - {getMachineTypeLabel(eq.type)}
                      </option>
                    ))}
                  </select>
                ...
                <div>
                  ...
                  <input
                    ...
                    readOnly={!!newOperation.equipment_id}
                  />
                  {newOperation.equipment_id && (
                    <p className="text-xs mt-1" style={{ color: "var(--color-muted-foreground)" }}>
                      Auto-rempli depuis l'équipement sélectionné
                    </p>
                  )}
                </div>
              </div>
