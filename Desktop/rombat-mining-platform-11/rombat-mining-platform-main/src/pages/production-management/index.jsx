...
export default function ProductionManagement() {
  ...
  const getWeeklyTotals = (records) => {
    return DIMENSIONS_LIST.reduce((totals, dim) => {
      totals[dim] = 0;
      return totals;
    }, {});
  };
  ...
  const formatDimensionLabel = (dimension) => {
    if (dimension === 'Minerai') return 'Nombre de voyages minerai';
    if (dimension === 'Forage') return 'Nombre de trous forés';
    return dimension;
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "var(--color-muted-foreground)" }}>
            Saisie production et suivi du stock par dimension
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="p-4 rounded-xl border" style={{ background: "var(--color-card)" }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm" style={{ color: "var(--color-muted-foreground)" }}>Tonnage Total</p>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b" style={{ borderColor: "var(--color-border)" }}>
                ...
                <th className="text-left p-4 text-sm font-medium" style={{ color: "var(--color-muted-foreground)" }}>Sorties de Stock</th>
                ...
                <tr key={index} className="border-b" style={{ borderColor: "var(--color-border)" }}>
                  ...
                  <td className="p-4">
                    <span className="px-2 py-1 rounded-full text-xs font-medium" 
                      style={{ 
                        background: item.available > 1 ? "rgba(34,197,94,0.12)" : "rgba(239,68,68,0.12)",
                        color: item.available > 1 ? "var(--color-success)" : "var(--color-error)"
                      }}>
                      {item.available > 1 ? 'Bon' : 'Critique'}
                    </span>
                  </td>
                </tr>
              ))}
                    ...
                    <div className="text-xs">
                      {item.dimensions.map(dim => (
                        <div key={dim.dimension} style={{ color: "var(--color-muted-foreground)" }}>
                          {formatDimensionLabel(dim.dimension)}: {dim.quantity}{dim.dimension === 'Minerai' || dim.dimension === 'Forage' ? '' : 't'}
                        </div>
                      ))}
                    </div>
                  </td>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 p-3 rounded-lg" style={{ background: "var(--color-muted)" }}>
                {['Minerai', 'Forage'].map(dimName => {
                  ...
                  const available = stockDim ? stockDim.available : 0;
                  const label = dimName === 'Minerai'
                    ? 'Nombre de voyages minerai'
                    : 'Nombre de trous fores';
                  return (
                    <div key={dimName}>
                      <label className="block text-sm font-semibold mb-0.5" style={{ color: "var(--color-foreground)" }}>{label}</label>
                      <p className="text-xs mb-1" style={{ color: "var(--color-muted-foreground)" }}>
                        Nombre disponible: <span style={{ fontWeight: 600, color: available > 0 ? "var(--color-success)" : "var(--color-muted-foreground)" }}>{available.toFixed(1)}</span>
                      </p>
                      <input
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    ...
                    <option value="auto_consumption">Auto consommation</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1" style={{ color: "var(--color-foreground)" }}>
